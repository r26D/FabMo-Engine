//TH Alternate LiveCode Runtime to try streaming motion from input devices; MODELED on gcoderuntime
var fs = require('fs');
...
LiveCodeRuntime.prototype.connect = function(machine) {
	...
	log.info("Connected LiveCode Runtime");
};

LiveCodeRuntime.prototype.disconnect = function() {
	if(this.ok_to_disconnect) {
		this.driver.removeListener('status', this.status_handler);
		log.info("Disconnected LiveCode Runtime");
	} else {
		throw new Error("Cannot disconnect LiveCode Runtime")
	}
};
...
LiveCodeRuntime.prototype._changeState = function(newstate) {
	if(newstate != "idle") {
...
LiveCodeRuntime.prototype._onDriverStatus = function(status) {
...
LiveCodeRuntime.prototype._die = function() {
...
LiveCodeRuntime.prototype._fail = function(message) {
	this.machine.status.current_file = null;
	this.machine.status.line=null;
	this.machine.status.nb_lines=null;
 	try {
...
LiveCodeRuntime.prototype._idle = function() {
	this.machine.status.current_file = null;
	this.machine.status.line=null;
	this.machine.status.nb_lines=null;
...
LiveCodeRuntime.prototype.executeCode = function(string, callback) {
